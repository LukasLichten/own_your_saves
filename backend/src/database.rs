...
use common::{U256, LargeU, data::{RequestUser, Device, TokenCarrier}};
...
pub fn set_key_value(conn: &Connection, key: String, value: String) {
    ...
    let _res = conn.execute(format!("
            ...
            ", key, value).as_str(), params![]);

    let _res = conn.cache_flush();
}

pub fn get_key_value(conn: &Connection, key: String) -> Option<String> {
    let res: Result<String, rusqlite::Error> = conn.query_row(format!("SELECT value FROM keyvalues WHERE key='{}'",key).as_str(), params![], |row| row.get(0));
    if let Ok(val) = res {
        return Some(val)
    }
    None
...
pub fn create_user(conn: &Connection, name: String, password: U256) -> bool {
    let res = conn.execute("INSERT INTO users (user_name, password) VALUES (?1, ?2)", (name.clone(), password.to_be_bytes()));

    if let Ok(_c) = res {
        let res:Result<u32, rusqlite::Error> = conn.query_row(format!("SELECT user_id FROM users WHERE user_name='{}'", name).as_str(), params![],|row| row.get(0));
...
pub fn login(conn: &Connection, name: String, password: U256, device_id: u8) -> Option<TokenCarrier> {
    let res:Result<(u32, [u8;32]), rusqlite::Error> = conn.query_row(format!("SELECT user_id, password FROM users WHERE user_name='{}'", name).as_str(), params![], |row| Ok((row.get(0)?, row.get(1)?)));
...
fn create_token(conn: &Connection, user_id: u32, device_id: u8) -> Uuid {
    ...
    if let Ok(rows) = res {
        ...
        let _res = conn.execute(format!("DELETE FROM tokens WHERE user_id='{}' AND device_id='{}' AND NOT token=x'{}'", user_id, device_id, common::bytes_to_hex_string(token.as_bytes())).as_str(), params![]);
